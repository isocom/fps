...
public class ThermalOld implements FiscalPrinter {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 0) {
            fp = ThermalOld.getFiscalPrinter(args[0]);
        } else {
            fp = ThermalOld.getFiscalPrinter("COM1");
        }
    ...
    public static FiscalPrinter getFiscalPrinter(String comPortName) {
        return new ThermalOld(comPortName);
    }
    /**
     ...
     * drukarka fiskalna.
     */
    private ThermalOld(String comPortName) {
    ...
    private void printSlip(Slip slip) throws FiscalPrinterException {
        ...
        //sendLBTRXEND(slip.getTotal(), slip.getCashBoxLogo(), slip.getCashierName(), slip.getExternalReference(), preparePayments(slip));
    ...
    private PaymentPosnetThermalOld preparePayments(Slip slip) {
        PaymentPosnetThermalOld retVal = new PaymentPosnetThermalOld();
    ...
    private void sendLBTRSLN(int slipLineNo, String name, double amount, double price, char vatRate, double gross) throws FiscalPrinterException {
        ...
        sendPrefix();
        try {
            //System.err.println("Linia do drukarki: " + new String(seq));
            outputStream.write(seq);
            outputStream.write(calculateCC(seq));
        } catch (IOException e) {
        }
    ...
    private void sendLBTREXIT(double total) throws FiscalPrinterException {
        ...
        if (err != 0) {
            throw new FiscalPrinterException("sendLBTREXIT " + this + " " + getErrDescription(err));
        }
    ...
    private void sendLBTRXEND(double total, String cashbox, String cashier, String reference, PaymentPosnetThermalOld payments) throws FiscalPrinterException {
        if (!printerConnected) {
            return;
        }
    ...
    public String toString() {
        String retValue = ThermalOld.class.getSimpleName() + " @ " + comPortName;
